...
DECLSPEC void crypt_derive_key_password_derivation_vector2 (sha1_hmac_ctx_vector_t *ctx, const u32x *w, const int len)
{
  u32x w0[4];
  u32x w1[4];
  u32x w2[4];
...
KERNEL_FQ void m19851_mxx (KERN_ATTR_VECTOR ())
{
  /**
   * base
   */
  ...
  for (u32 il_pos = 0; il_pos < IL_CNT; il_pos += VECT_SIZE)
  {
    ...
    sha1_hmac_ctx_vector_t sha1_hmac_ctx_vec2;
    crypt_derive_key_password_derivation_vector2(&sha1_hmac_ctx_vec2, w, pw_len);

    u32 aes_key[8];

    aes_key[0] = sha1_hmac_ctx_vec.ipad.h[0].s0;
    aes_key[1] = sha1_hmac_ctx_vec.ipad.h[1].s0;
    aes_key[2] = sha1_hmac_ctx_vec.ipad.h[2].s0;
    aes_key[3] = sha1_hmac_ctx_vec.ipad.h[3].s0;
    aes_key[4] = sha1_hmac_ctx_vec.ipad.h[4].s0;
    aes_key[5] = sha1_hmac_ctx_vec2.ipad.h[0].s0;
    aes_key[6] = sha1_hmac_ctx_vec2.ipad.h[1].s0;
    aes_key[7] = sha1_hmac_ctx_vec2.ipad.h[2].s0;
...
KERNEL_FQ void m19851_sxx (KERN_ATTR_VECTOR ())
{
  /**
   * base
   */
